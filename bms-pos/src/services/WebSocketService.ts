...
// Event handler type
export type EventHandler = (_: BMSWebSocketEvent) => void | Promise<void>;

// Sync status event handler
export type SyncStatusHandler = (_: SyncStatus & { websocketState?: ConnectionState }) => void;
...
export class POSWebSocketService {
  ...
  async connect(namespace: 'main' | 'admin' | 'pos' = 'pos'): Promise<Socket> {
    if (this.socket?.connected) {
      Logger.info('POS WebSocket already connected')
      return this.socket
    }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.socket.on('connect', async () => {
          Logger.info(`🚀 POS WebSocket connected to ${namespacePath}`)
        ...
        this.socket.on('connect_error', (error) => {
          Logger.error('POS WebSocket connection error:', error)
          this.connectionState = 'error'
          reject(error)
        })

        this.socket.on('disconnect', (reason) => {
          Logger.info('POS WebSocket disconnected:', reason)
          this.handleDisconnection(reason)
        })

        this.socket.on('error', (error) => {
          Logger.error('POS WebSocket error:', error)
          this.connectionState = 'error'
        })
      })

    } catch (error) {
      Logger.error('Failed to connect POS WebSocket:', error)
      this.connectionState = 'error'
      throw error
    }
  }
  ...
  private async joinPOSRooms(): Promise<void> {
    ...
    try {
      ...
      Logger.error('Error joining POS rooms:', error)
    }
  }
  ...
  private setupSocketListeners(): void {
    ...
    this.socket.on('reconnect', () => {
      Logger.info('🔄 POS WebSocket reconnected')
      this.connectionState = 'connected'
      this.notifyConnectionStateListeners()
    })

    this.socket.on('reconnect_attempt', (attemptNumber) => {
      Logger.info(`🔄 POS reconnection attempt ${attemptNumber}`)
      this.connectionState = 'reconnecting'
      this.notifyConnectionStateListeners()
    })

    this.socket.on('reconnect_error', (error) => {
      Logger.error('POS reconnection error:', error)
      this.connectionState = 'error'
      this.notifyConnectionStateListeners()
    })

    this.socket.on('reconnect_failed', () => {
      Logger.error('POS reconnection failed')
      this.connectionState = 'error'
      this.notifyConnectionStateListeners()
    })
      ...
      if (this.config.syncServiceIntegration) {
        setTimeout(() => {
          syncService.syncProductsOnly().catch(error => {
            Logger.error('Failed to sync products after update:', error)
          })
        }, 1000)
      }
    })
    ...
    this.socket.on('inventory:updated', (event: BMSWebSocketEvent) => {
      Logger.info('📋 Inventory update received for POS:', event.data)
      ...
      if (this.config.syncServiceIntegration) {
        setTimeout(() => {
          syncService.syncProductsOnly().catch(error => {
            Logger.error('Failed to sync inventory after update:', error)
          })
        }, 1000)
      }
    })
    ...
    this.socket.on('system:notification', (event: BMSWebSocketEvent) => {
      Logger.info('🔔 System notification received:', event.data)
      this.emitEvent('system:notification', event)
    })

    this.socket.on('low-stock:alert', (event: BMSWebSocketEvent) => {
      Logger.info('⚠️ Low stock alert received:', event.data)
      this.emitEvent('low-stock:alert', event)
    })

    // Room events
    this.socket.on('room-joined', (data) => {
      Logger.info(`📍 POS joined room: ${data.roomId}`)
    })

    this.socket.on('room-left', (data) => {
      Logger.info(`📍 POS left room: ${data.roomId}`)
    })

    this.socket.on('rooms-list', (data) => {
      Logger.info(`📍 POS current rooms: ${data.rooms.join(', ')}`)
    })

    // Error handling
    this.socket.on('error', (error) => {
      Logger.error('POS WebSocket server error:', error)
    ...
    this.socket.on('force-disconnect', (data) => {
      Logger.info('🔌 POS force disconnect:', data.reason)
  ...
  private handleSyncStatusUpdate(event: BMSWebSocketEvent): void {
    try {
      ...
      Logger.error('Error handling sync status update:', error)
    }
  }
  ...
  private handleDisconnection(reason: string): void {
    ...
    if (reason === 'io server disconnect') {
      Logger.info('🔌 POS disconnected by server')
    } else if (reason === 'io client disconnect') {
      Logger.info('🔌 POS disconnected by client')
    } else if (reason === 'transport close') {
      Logger.info('🔌 POS transport closed')
    } else if (reason === 'ping timeout') {
      Logger.info('🔌 POS ping timeout')
    }
  ...
  private scheduleReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      Logger.error('❌ POS max reconnection attempts reached')
      this.connectionState = 'error'
      this.notifyConnectionStateListeners()
      return
    }
    ...
    Logger.info(`⏰ POS scheduling reconnection in ${delay}ms (attempt ${this.reconnectAttempts})`)
    
    setTimeout(() => {
      if (!this.socket?.connected && !this.isIntentionallyDisconnected) {
        Logger.info('🔄 POS attempting reconnection...')
        this.socket?.connect()
      }
    }, delay)
  }
    ...
    if (handlers) {
      handlers.forEach(handler => {
        try {
          ...
          Logger.error(`Error in POS event handler for ${eventType}:`, error)
        }
      })
    }
  }
  ...
  private notifySyncStatusHandlers(status: SyncStatus & { websocketState?: ConnectionState }): void {
    this.syncStatusHandlers.forEach(handler => {
      try {
        ...
        Logger.error('Error in POS sync status handler:', error)
      }
    })
  }

  // Connection state management
  private connectionStateListeners: Set<(_: ConnectionState) => void> = new Set()

  onConnectionStateChange(listener: (_: ConnectionState) => void): () => void {
  ...
  private notifyConnectionStateListeners(): void {
    this.connectionStateListeners.forEach(listener => {
      try {
        ...
        Logger.error('Error in POS connection state listener:', error)
      }
    })
  }
  ...
  async triggerSyncWithNotification(): Promise<void> {
    try {
      Logger.info('🔄 POS triggering manual sync with WebSocket notification')
      ...
      Logger.error('Failed to trigger sync with notification:', error)
