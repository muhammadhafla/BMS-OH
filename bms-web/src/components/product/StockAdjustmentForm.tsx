...
export function StockAdjustmentForm({ product, open, onOpenChange, onSuccess }: StockAdjustmentFormProps) {
  ...
  const getStockStatus = (stock: number) => {
    ...
    } else if (stock >= product.maxStock) {
  ...
  const onSubmit = async (data: StockAdjustmentFormData) => {
    ...
    try {
      ...
      const response = await apiService.updateProductStock(product.id, adjustmentData);
      ...
      if (response.success) {
        ...
        const description = needsApproval
          ? `Adjustment for ${product.name} is pending approval.`
          : `Stock for ${product.name} has been updated.`;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Adjust the stock level for {product.name} (SKU: {product.sku})
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Branch: {product.branch.name} • Min: {product.minStock} • Max: {product.maxStock}
                  </p>
                </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between border-t pt-2">
                        ...
                        <div className="text-right">
                          <span className="font-bold text-lg">{newStock} {product.unit}</span>
